...
def autosave_fig(base_name, root = '../../reports/figures/'):
    '''
    Helper function to automatically save rendered figure as SVG
    with an autogenerated name. This function will scan the 'root'
    directory, find the number of figures already save, and
    increment the filename automatically.

    Args:
        base_name (str): base name of saved figure, an interger
                         will be appended to this.
        root (str): directory to scan for existing figures, this
                    should be run w.r.t to the jupyter notebook

    Yields:
        confirmation of saved file
    '''
    current_figs = [l for l in os.listdir(root) if base_name in l]
    count = len(current_figs)
    fig_name = "%s-%s.svg" %(base_name, count)
